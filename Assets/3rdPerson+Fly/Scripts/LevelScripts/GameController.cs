...
public class GameController : MonoBehaviour
{
	...
	public bool _searcher = false;

	//Cosas de tarjetas
	public GameObject PanelBusinessCards;
	public bool _panelBusinessCards = false;

	//Cosas Notificaciones
	public GameObject Notificaciones;
	public bool _notifications = false;
    ...
    public void Abrir_CerrarTarjetas() {
		if (!_panelBusinessCards)
		{
			CerraPestañasAbiertas();
			PanelBusinessCards.GetComponent<Animator>().SetTrigger("Open");
			PanelBusinessCards.GetComponent<Animator>().SetBool("OpenB", true);
			BarraPresionado[2].SetActive(true);
			NuevasNotificaciones[0].SetActive(false);
			_panelBusinessCards = true;
		}
		else if (_panelBusinessCards)
		{
			PanelBusinessCards.GetComponent<Animator>().SetBool("OpenB", false);
			BarraPresionado[2].SetActive(false);
			NuevasNotificaciones[0].SetActive(false);
			_panelBusinessCards = false;
		}
	}
    #endregion
	...
	public bool CheckAllClose() {
		if (!_searcher && !tutorialActive && !_panelBusinessCards && !_notifications && !mapaActive && !misDatosActive && !conferenciasActive)
		{
			return true;
		}
		else
